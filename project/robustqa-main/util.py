...
class QADataset(Dataset):
    def __init__(self, encodings, train=True):
        ...
        if train:
            self.keys += ['domain_label', 'start_positions', 'end_positions']
...
def read_squad(path, domain_index):
    path = Path(path)
    with open(path, 'rb') as f:
        squad_dict = json.load(f)
    data_dict = {'question': [], 'context': [], 'id': [], 'answer': [], 'domain_label':[]}
    for group in squad_dict['data']:
        for passage in group['paragraphs']:
            context = passage['context']
            for qa in passage['qas']:
                question = qa['question']
                # TODO : why are we considering questions without answers?
                # TODO : no supervision is coming from them.
                if len(qa['answers']) == 0:
                    data_dict['question'].append(question)
                    data_dict['context'].append(context)
                    data_dict['id'].append(qa['id'])
                    data_dict['domain_label'].append(domain_index)
                else:
        ...
        assert(len(data_dict['question']) == len(data_dict['domain_label']))
    
    #print("read_squad_0", len(data_dict['answer']), len(data_dict['question']))

    id_map = ddict(list)
    for idx, qid in enumerate(data_dict['id']):
        id_map[qid].append(idx)
        # gathers all indices for a ID

    data_dict_collapsed = {'question': [], 'context': [], 'id': [], 'domain_label': []}
    if data_dict['answer']:
        data_dict_collapsed['answer'] = []
    for qid in id_map:
        ex_ids = id_map[qid] # indices for this ID
        ...
        data_dict_collapsed['domain_label'].append(data_dict['domain_label'][ex_ids[0]])
    ...
    #print("read_squad", len(data_dict['answer']), len(data_dict['question']))
